...
const REMOVES_FROM_QUEUE = new Set(["approve", "reject"]);
...
export function QueueListClient({ items }: { items: QueueItem[] }): JSX.Element {
  ...
  function onDecisionClick(e: MouseEvent<HTMLButtonElement>): void {
    const decision = e.currentTarget.value;
    if (!REMOVES_FROM_QUEUE.has(decision)) return;
    const tecnicoId = e.currentTarget.dataset.tecnicoId;
    if (!tecnicoId) return;
    // Add to removed BEFORE the form submits so the transition starts the
    // moment HR clicks. The native form action then submits as usual.
    setRemoved((s) => {
      const n = new Set(s);
      n.add(tecnicoId);
      return n;
    ...
    <>
      ...
      {items.length === 0 ? (
        ...
        <ul className="space-y-3 mt-6">
          {items.map((tec) => {
            const isRemoved = removed.has(tec.tecnico_id);
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <form action={submitDecision} className="flex flex-col gap-2 shrink-0 w-44">
                    ...
                    <button
                      ...
                      name="decision"
                      value="approve"
                      data-tecnico-id={tec.tecnico_id}
