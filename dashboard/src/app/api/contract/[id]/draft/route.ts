// Contract draft PDF generator. Renders a Colombian "prestación de servicios"
...
//
// Reference: Código Civil Colombiano + Art. 34 CST. Phase-1 contract is a
// civil (independent contractor) instrument — the worker pays own SS, ARL
// (Clase IV o V), and the contracting party (Redin) bears no employer-side
// obligations. The 4-year max duration and 40% min IBC are 2025 normative
// changes; we hold to both. The PDF is a draft until both signatures land.
...
const styles = StyleSheet.create({
  ...
  signatures: { marginTop: 24, flexDirection: "row", justifyContent: "space-between", gap: 32 },
  sigBlock: { flex: 1 },
  sigLine: { borderTopWidth: 1, borderTopColor: "#0f172a", marginTop: 28, marginBottom: 4 },
  foot: { marginTop: 18, fontSize: 8.5, color: "#475569", borderTopWidth: 1, borderTopColor: "#e2e8f0", paddingTop: 8 },
});
...
function ContractDoc(p: Props) {
  ...
  const objetoBase = p.descripcion?.trim() || "los servicios técnicos descritos en la OT y su cotización aprobada";
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  // Pull técnico nombre from the latest tecnico_registered event meta — same
